...
import com.spring.edu.vo.PhotoVo;
...
public class EditorController {
    ...
    /*public void file_uploader_html5(HttpServletRequest request,HttpServletResponse response) throws Exception {
        //파일정보
        String sFileInfo = "";
        //파일명을 받는다 - 일반 원본파일명
        String filename = request.getHeader("file-name");
        ...
        //이미지가 아님
        if (cnt == 0) {
            System.out.println("파일정렬NOTALLOW_" + filename);
        } else {
            ...
            sFileInfo += "&bNewLine=true";    
            sFileInfo += "&sFileName=" + filename;    
            sFileInfo += "&sFileURL=http://localhost:8787/resources/editor/sample/photo_uploader/upload/"+realFileNm;
            System.out.println("파일이 왜 안나올까 : "+ filename);
        }
        return;
